...
ez::tracking_wheel horiz_tracker(3, 2.75, 1.0);             // Rotation sensors
...
void initialize() {
  ...
  ez::as::initialize();
  ez::as::limit_switch_lcd_initialize(&increase);
  ...
  ez::as::auton_selector.autons_add({
    Auton("red 6 ring", red_left_6ring),
    Auton("skills", skills),
    Auton("red solo awp", red_solo_awp),
    Auton("red right arm rush", red_right_rush),
    Auton("intake test", intake_test),
    Auton("lift test", lift_test),
...
void opcontrol() {
  ...
  while (true) {
    ...
    arm.button_toggle(master.get_digital(DIGITAL_Y));
    intakelift.button_toggle(master.get_digital(DIGITAL_X));
     ...
     // Lift PID Control
        ...
        // Scale position for PID
        // double scaled_position = rotation.get_position() / 3.0;  // Account for gear ratio
